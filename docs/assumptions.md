...
| T7 | **The ingest script connects as the bootstrap superuser** (`INGEST_DATABASE_URL`, same role/credential as `migrate`), not `app_user`. | 🗣️ Owner-supplied | Both RLS policies (migration 001) are `USING`-only, so they apply to `INSERT` as well as `SELECT` — an `app_user` connection would need `SET LOCAL app.current_customer` re-issued per customer group inside the batch transaction to write at all. Connecting as the superuser bypasses RLS unconditionally instead: the second legitimate non-RLS-scoped path in the system, alongside `/customers/top` (T5). If this changes, `ingest_rejects` (I10) needs re-deciding too, since it currently piggybacks on "ingestion never runs as `app_user`". |
...
| I2 | **Resolved.** `metadata` is excluded from the hash, *and* the insert uses `ON CONFLICT ... DO UPDATE` (not `DO NOTHING`) on `duration_ms`/`status`/`metadata`. A redelivery with a corrected `duration_ms` hits the same key and overwrites the original — the correction is applied, not discarded. See [ADR 0005](adr/0005-derived-dedupe-key-no-source-event-id.md) for the three-option comparison. | 🗣️ Owner-supplied | If "latest wins" is wrong (e.g. the *first* delivery should be authoritative), swap `DO UPDATE` for `DO NOTHING` — a one-line change, isolated to `src/ingest/ingestEvents.ts`. |
...
| I8 | `duration_ms` that is non-integer, negative, or absurdly large is rejected; **absent** `duration_ms` is not (see D6). An explicit `duration_ms: null` is treated identically to an absent key — same "no duration recorded" meaning, not malformed. | 🔍 Inferred | Conflating "absent" with "malformed" would reject every valid `login` event. The explicit-`null` fixture record (found running the full synthetic fixture, not by inspection) is exactly this trap in a different serialization. |
...
| I10 | `ingest_rejects` is **not RLS-scoped** — no policy, and `app_user` gets no grant on it at all (access control is privilege-based: deny by omission). | 🔍 Inferred (follows T7 + I6) | I6 means reject rows routinely have a NULL/garbage `customer_id`, which a `customer_id = current_setting(...)` policy can't cleanly express (it would make exactly those rows unwritable). This is only safe *because* ingestion never connects as `app_user` (T7) — if that changes, this table needs an actual policy, not just an omitted grant. |
| I11 | Reject rows are **not deduped across ingestion runs** — re-running the loader against an unfixed file re-quarantines the same malformed records every time, growing the table unboundedly on repeated reruns. | ⚠️ Invented | Deliberate: a reject reflects a processing attempt, not a billable fact, so re-surfacing it isn't wrong the way a duplicate billable event would be. But an operator repeatedly re-running ingestion against the same bad file (e.g. in a retry loop) would see `ingest_rejects` grow without bound and the reject-rate metric (A6) drift upward per-run rather than per-file. Worth a cleanup/retention policy before this ran unattended in production. |
...
1. ~~Is a redelivered event with different `metadata` the same billable event?~~ (I2) — **resolved**: same event, latest delivery's `metadata` wins. Still worth confirming with the owner before this ships, since "latest wins" was my call among three defensible options, not a spec-derived certainty.
